...
function CameraFeedGen1({ currentColor, onPrediction, predictionService, useWebSocket = true, cameraEnabled = true }) {
        ...
        if (sequenceBufferRef.current.length === SEQ_LEN &&
            ...
            if (timeSinceLastPrediction >= 1000) {
                ...
                try {
                    ...
                    setBufferSize(0);
                    console.log('🔄 [GEN1] Buffer cleared - starting fresh');
    ...
    useEffect(() => {
        if (!cameraEnabled) {
            console.log('📷 [GEN1] Camera disabled, waiting for manual start');
            return;
        }

        const initCamera = async () => {
            if (!videoRef.current) {
                console.error('❌ [GEN1] Video ref not available');
                return;
            }
            ...
            try {
                ...
                console.log('✅ [GEN1] Holistic camera initialized');
            } catch (error) {
                console.error('❌ [GEN1] Camera initialization error:', error);
            }
        };
        ...
        return () => {
            console.log('🛑 [GEN1] Cleaning up camera');
