...
const SEQ_LEN = 30; // Sequence length for motion recognition (REQUIRED by model)
const FACE_LM = 468;
const POSE_LM = 33;
const HAND_LM = 21;
const PROCESS_EVERY_N_FRAMES = 2; // Process every 2nd frame for faster performance

function CameraFeedColors({ currentColor, onPrediction, predictionService, useWebSocket = true, cameraEnabled = true }) {
    const videoRef = useRef(null);
    const canvasRef = useRef(null);
    const cameraRef = useRef(null);
    const holisticRef = useRef(null);
    const sequenceBufferRef = useRef([]);  // Use ref instead of state for better performance
    const [bufferSize, setBufferSize] = useState(0);  // Just for UI display
        ...
        if (sequenceBufferRef.current.length === SEQ_LEN &&
            ...
            if (timeSinceLastPrediction >= 1000) {
                ...
                try {
                    ...
                    setBufferSize(0);
                    console.log('🔄 [COLORS] Buffer cleared - starting fresh');
    ...
    useEffect(() => {
        if (!cameraEnabled) {
            console.log('📷 [COLORS] Camera disabled, waiting for manual start');
            return;
        }

        const initCamera = async () => {
            if (!videoRef.current) {
                console.error('❌ [COLORS] Video ref not available');
                return;
            }
            ...
            try {
                ...
                console.log('✅ [COLORS] Holistic camera initialized');
            } catch (error) {
                console.error('❌ [COLORS] Camera initialization error:', error);
            }
        };
        ...
        return () => {
            console.log('🛑 [COLORS] Cleaning up camera');
    ...
    }, [cameraEnabled]); // Add cameraEnabled dependency
...
export default CameraFeedColors;
