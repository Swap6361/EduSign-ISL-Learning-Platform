...
const CameraFeedDays = ({ currentDay, onPrediction, useWebSocket = true }) => {
    ...
    // Use the imported predictionServiceDays
    const predictionService = useWebSocket ? predictionServiceDays : null;

    const startCamera = async () => {
        console.log('🎥 Starting camera for days...');
        setIsLoading(true);
        setError(null);
    ...
    const initializeMediaPipe = () => {
        ...
        console.log('✅ MediaPipe initialized for days');
    };
    ...
    const handleResults = (results) => {
        ...
        if (readyToSend) {
            lastSendTime = now;
            console.log('📤 [DAYS] Sending features to backend');
            console.log('  ├─ Feature length:', features.length);
            console.log('  ├─ Stable:', stable);
            console.log('  ├─ Target day:', currentDay);
            console.log('  ├─ Non-zero ratio:', (nonZeroRatio * 100).toFixed(1) + '%');
            console.log('  ├─ Hands detected:', { left: leftDetected, right: rightDetected });
            console.log('  └─ Service available:', !!predictionService);

            if (predictionService && predictionService.sendLandmarks) {
                try {
                    predictionService.sendLandmarks(features, currentDay);
                    console.log('✅ [DAYS] Landmarks sent successfully');
                } catch (err) {
                    console.error('❌ [DAYS] Error sending landmarks:', err);
                }
            } else {
                console.error('❌ [DAYS] Prediction service not available or missing sendLandmarks method');
            }
        }
    };

    useEffect(() => {
        console.log('🔌 [DAYS] Setting up prediction listener');

        const handlePred = (result) => {
            console.log('📥 [DAYS] Prediction received:', result);
            if (!result.uiOnly && result.success && onPrediction) {
                console.log('✅ [DAYS] Forwarding prediction to parent:', result.day || result.label);
                onPrediction(result);
            } else if (!result.success) {
                console.warn('⚠️ [DAYS] Prediction failed:', result.error);
            }
        };

        if (predictionService && predictionService.onPrediction) {
            predictionService.onPrediction(handlePred);
            console.log('✅ [DAYS] Prediction listener registered');
        } else {
            console.error('❌ [DAYS] Cannot register prediction listener - service unavailable');
        }

        return () => {
            if (predictionService && predictionService.offPrediction) {
                predictionService.offPrediction(handlePred);
                console.log('🔌 [DAYS] Prediction listener removed');
            }
        };
    }, [onPrediction, predictionService, currentDay]);
